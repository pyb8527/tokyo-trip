...
로그인·장소·가계부·정산을 담당하는 API 입니다. **의존성이 하나도 없습니다** —
Node 24 내장 모듈만 씁니다 (`node:http`, `node:sqlite`, `node:crypto`).
`npm install` 이 필요 없고 네이티브 모듈 컴파일도 없습니다.

```
server/
  package.json      의존성 0개
  Dockerfile        node:24-alpine
  src/db.js         스키마 · 마이그레이션
  src/auth.js       scrypt 해시 · 세션 · 시도 제한
  src/http.js       라우터 · 쿠키 · 본문 파싱
  src/routes.js     API
  src/index.js      진입점
  src/seed.js       index.html 일정 → DB
```
...
# 설치 토큰 만들기 (최초 관리자 계정을 만들 때 한 번만 쓰입니다)
cp .env.example .env
openssl rand -base64 32          # 결과를 .env 의 SETUP_TOKEN 에 넣으세요
vi .env
...
브라우저로 접속하면 **첫 관리자 만들기** 화면이 뜹니다. 설치 토큰·이메일·이름·비밀번호를
넣으면 관리자 계정과 여행이 만들어집니다. **계정이 하나라도 생기면 이 경로는 스스로 잠깁니다.**

그다음 지금 `index.html` 에 들어 있는 일정을 DB 로 옮깁니다:

```bash
sudo docker compose exec api node src/seed.js
# 이미 장소가 있는데 덮어쓰려면
...
만들면 자동으로 이번 여행의 동행자가 되고, 동행자는 일정을 고치고 지출을 남길 수 있습니다.

### 보안

| | |
|---|---|
| 비밀번호 | scrypt N=2¹⁵ · 1회 약 90ms. 평문은 어디에도 남기지 않습니다 |
| 세션 | 토큰 원본을 저장하지 않고 sha256 만 저장. DB 가 새도 그것만으로는 로그인 불가 |
| 계정 열거 | 로그인 실패 응답을 하나로 통일 |
| 무차별 대입 | IP+이메일당 15분에 10회 |
| CSRF | SameSite=Lax + 커스텀 헤더 이중 |
| 최초 관리자 | `SETUP_TOKEN` 일치 시 1회. 계정이 생기면 잠김 |
| 관리자 보호 | 활성 관리자가 0명이 되는 강등·비활성·삭제를 막습니다 |

### 백업

SQLite 파일 하나가 전부입니다. `api-data` 볼륨만 챙기면 됩니다.

```bash
sudo docker compose exec api sh -c 'cat /app/data/trip.db' > trip-backup-$(date +%F).db
```

### 서버가 죽어도

**일정은 그대로 보입니다.** 화면은 서버에 못 닿으면 마지막으로 받은 일정을
브라우저 저장소에서 읽고, 그것도 없으면 `index.html` 에 들어 있는 기본 일정을 씁니다.
이 경우 드로어에 "서버에 연결되지 않아 저장된 일정만 표시합니다" 라고 뜨고
편집 메뉴는 사라집니다.
...
| **로그인** | 서버가 붙어 있으면 로그인해야 봅니다. 서버가 없으면 로그인 없이 그대로 보입니다 |
| **메뉴** | 좌측 햄버거 드로어 + 하단 바로가기(일정·지도·가계부·더보기) |
| **장소 관리** | 추가·수정·삭제. 좌표는 지도 클릭 또는 구글 지도 링크 붙여넣기. 시간을 넣으면 자동 정렬 |
| **동행자** | 계정별 방문 기록, 지출의 낸 사람·나눠 낼 사람 지정, 최소 횟수 정산 |
| **관리자** | `admin.html` — 계정 생성·역할 변경·비활성·삭제, 비밀번호 재설정, 활동 기록 |
